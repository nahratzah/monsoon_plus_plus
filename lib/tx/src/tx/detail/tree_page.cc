...
auto abstract_tree::equal_range(const abstract_tree_page_branch_key& key) -> std::pair<abstract_tree_iterator, abstract_tree_iterator> {
  std::shared_lock<std::shared_mutex> lck{ mtx_ };
  if (root_off_ == 0) return { end(), end() };
  page_with_lock pwl = get(root_off_);
  lck.unlock();

  // Descend down the tree of pages.
  while (const auto branch_page = std::dynamic_pointer_cast<tree_page_branch>(pwl.page)) {
    std::shared_ptr<const abstract_tree_page_branch_elem> found;
    // Branch pages don't have nil elements.
    assert(std::none_of(branch_page->elems_.begin(), branch_page->elems_.end(), [](const auto& ptr) { return ptr == nullptr; }));
    assert(std::none_of(branch_page->keys_.begin(), branch_page->keys_.end(), [](const auto& ptr) { return ptr == nullptr; }));
    ...
    if (key_range.first == key_range.second) { // Single page descent.
      pwl = get(branch_page->elems_[key_range.first - branch_page->keys_.begin()]->off);
    } else {
      // Split case is similar to running the lower and upper bound separately.
      auto b = lower_bound(get(branch_page->elems_[key_range.first - branch_page->keys_.begin()]->off), key);
      pwl = get(branch_page->elems_[key_range.second - branch_page->keys_.begin()]->off);
      auto e = upper_bound(std::move(pwl), key);
      return std::make_pair(std::move(b), std::move(e));
    }
  }

  // We made it all the way to a single leaf page.
  auto leaf_page = boost::polymorphic_pointer_downcast<tree_page_leaf>(pwl.page);
  auto b = leaf_page->elems_.begin();
  while (b != leaf_page->elems_.end() && (*b == nullptr || less_cb(**b, key))) ++b;

  if (b == leaf_page->elems_.end()) [[unlikely]] {
    // There are no elements on this page, and we know the successor pages can't hold a range of items for us.
    for (pwl = leaf_page = leaf_page->next();
        leaf_page != nullptr;
        pwl = leaf_page = leaf_page->next()) {
      b = std::find_if(leaf_page->elems_.begin(), leaf_page->elems_.end(), [](const auto& ptr) { return ptr != nullptr; });
      if (b != leaf_page->elems_.end()) [[likely]] {
        return std::make_pair(
          abstract_tree_iterator(shared_from_this(this), *b),
          abstract_tree_iterator(shared_from_this(this), *b));
      }
    }
    return { end(), end() };
  ...
  auto e = b;
  while (e != leaf_page->elems_.end() && (*e == nullptr || !less_cb(key, **e))) ++e;
  if (e != leaf_page->elems_.end()) [[likely]] {
    return std::make_pair(
      abstract_tree_iterator(shared_from_this(this), *b),
      abstract_tree_iterator(shared_from_this(this), *e));
  }

  // We didn't find an upper bound on this page, so find the very first element on successor pages.
  // Note: we keep pwl unchanged since it keeps our iterator valid.
  page_with_lock spwl;
  for (spwl = leaf_page = leaf_page->next();
      leaf_page != nullptr;
      spwl = leaf_page = leaf_page->next()) {
    e = std::find_if(leaf_page->elems_.begin(), leaf_page->elems_.end(), [](const auto& ptr) { return ptr != nullptr; });
    if (e != leaf_page->elems_.end()) [[likely]] {
      return std::make_pair(
        abstract_tree_iterator(shared_from_this(this), *b),
        abstract_tree_iterator(shared_from_this(this), *e));
  ...
  // No successor holds an element, so we use the end() of the tree.
  return std::make_pair(
    abstract_tree_iterator(shared_from_this(this), *b),
    end());
}

auto abstract_tree::lower_bound(const abstract_tree_page_branch_key& key) -> abstract_tree_iterator {
  std::shared_lock<std::shared_mutex> lck{ mtx_ };
  if (root_off_ == 0) return end();
  page_with_lock pwl = get(root_off_);
  lck.unlock();

  return lower_bound(std::move(pwl), key);
}

auto abstract_tree::upper_bound(const abstract_tree_page_branch_key& key) -> abstract_tree_iterator {
  std::shared_lock<std::shared_mutex> lck{ mtx_ };
  if (root_off_ == 0) return end();
  page_with_lock pwl = get(root_off_);
  lck.unlock();

  return upper_bound(std::move(pwl), key);
}

auto abstract_tree::lower_bound(page_with_lock pwl, const abstract_tree_page_branch_key& key) -> abstract_tree_iterator {
  // Descend down the tree of pages.
  while (const auto branch_page = std::dynamic_pointer_cast<tree_page_branch>(pwl.page)) {
    std::shared_ptr<const abstract_tree_page_branch_elem> found;
    // Branch pages don't have nil elements.
    ...
    const auto key_index = std::lower_bound(branch_page->keys_.begin(), branch_page->keys_.end(), &key,
        [this](const auto& x, const auto& y) -> bool {
          return less_cb(*x, *y);
        });
    pwl = get(branch_page->elems_[key_index - branch_page->keys_.begin()]->off);
  }
  ...
  for (auto leaf_page = boost::polymorphic_pointer_downcast<tree_page_leaf>(pwl.page);
    ...
    auto b = leaf_page->elems_.begin();
    while (b != leaf_page->elems_.end() && (*b == nullptr || less_cb(**b, key))) ++b;
    if (b != leaf_page->elems_.end()) return abstract_tree_iterator(shared_from_this(this), *b);
  }
  // Ran out of pages.
  return end();
}

auto abstract_tree::upper_bound(page_with_lock pwl, const abstract_tree_page_branch_key& key) -> abstract_tree_iterator {
  // Descend down the tree of pages.
  while (const auto branch_page = std::dynamic_pointer_cast<tree_page_branch>(pwl.page)) {
    std::shared_ptr<const abstract_tree_page_branch_elem> found;
    // Branch pages don't have nil elements.
    ...
    const auto key_index = std::upper_bound(branch_page->keys_.begin(), branch_page->keys_.end(), &key,
        [this](const auto& x, const auto& y) -> bool {
          return less_cb(*x, *y);
        });
    pwl = get(branch_page->elems_[key_index - branch_page->keys_.begin()]->off);
  ...
  for (auto leaf_page = boost::polymorphic_pointer_downcast<tree_page_leaf>(pwl.page);
      leaf_page != nullptr;
      pwl = leaf_page = leaf_page->next(pwl.lck)) {
    auto e = leaf_page->elems_.begin();
    while (e != leaf_page->elems_.end() && (*e == nullptr || !less_cb(key, **e))) ++e;
    if (e != leaf_page->elems_.end()) return abstract_tree_iterator(shared_from_this(this), *e);
  }
  // Ran out of pages.
  return end();
}
