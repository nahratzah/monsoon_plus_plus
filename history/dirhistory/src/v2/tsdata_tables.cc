...
instrumentation::timing tsdata_v2_tables_decode_timing(
    instrumentation::timing::cumulative(
        "monsoon.dirhistory.decode",
        {
          {"file_type", "tsdata_v2"},
          {"operation", "column_read"}
        }));
instrumentation::timing tsdata_v2_tables_less_timing(
    instrumentation::timing::cumulative(
        "monsoon.dirhistory.decode",
        {
          {"file_type", "tsdata_v2"},
...
instrumentation::timing tsdata_v2_tables_merge_timing(
    instrumentation::timing::cumulative(
        "monsoon.dirhistory.decode",
        {
          {"file_type", "tsdata_v2"},
...
instrumentation::timing tsdata_v2_tables_new_objpipe_timing(
    "monsoon.dir_history.decode",
    {
      {"file_type", "tsdata_v2"},
      {"operation", "new_objpipe"}
    });

auto emit_type_less = [](const tsdata_v2_tables::emit_type& x, const tsdata_v2_tables::emit_type& y) noexcept -> bool {
  instrumentation::time_track<instrumentation::timing> tt{ tsdata_v2_tables_less_timing };
...
auto emit_type_merge = [](tsdata_v2_tables::emit_type&& x, tsdata_v2_tables::emit_type&& y) -> tsdata_v2_tables::emit_type&& {
  instrumentation::time_track<instrumentation::timing> tt{ tsdata_v2_tables_merge_timing };
...
-> decltype(auto) {
  ...
  return objpipe::new_callback<emit_type>(
      [=](auto cb) {
        instrumentation::time_track<instrumentation::timing> tt{ tsdata_v2_tables_decode_timing };
...
-> objpipe::reader<emit_type> {
  ...
  if (is_sorted() && is_distinct()) { // Operate on sequential blocks.
    static instrumentation::counter stat("monsoon.dirhistory.tsdata_v2_tables.emit", { {"style", "linear"} });
    ++stat;
    return block_chain.iterate();
  } else { // Parallel iteration of blocks.
    if (is_distinct()) { // Merge only.
      static instrumentation::counter stat("monsoon.dirhistory.tsdata_v2_tables.emit", { {"style", "distinct_merge"} });
      ++stat;
      ...
      static instrumentation::counter stat("monsoon.dirhistory.tsdata_v2_tables.emit", { {"style", "full_merge"} });
      ++stat;
      return objpipe::merge_combine(
          block_chain.begin(),
          block_chain.end(),
